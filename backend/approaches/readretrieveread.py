...
class CustomHtmlCallbackHandler:
    def __init__(self):
        self.log = []
    ...
    def get_and_reset_log(self):
        log_content = "\n".join(self.log)
        self.log = []  # Reset log for the next use
        return log_content
...
Thought: {agent_scratchpad}"""
    ...
    async def retrieve(self, query_text: str, overrides: dict[str, Any], auth_claims: dict[str, Any]) -> Any:
        ...
        if use_semantic_captions:
            results = [
                doc[self.sourcepage_field] + ":" + remove_newlines(" -.- ".join([c.text for c in doc["@search.captions"]]))
                async for doc in r
            ]
        else:
            results = [doc[self.sourcepage_field] + ":" + remove_newlines(doc[self.content_field][:250]) async for doc in r]
        content = "\n".join(results)
    ...
    async def run(self, q: str, overrides: dict[str, Any], auth_claims: dict[str, Any]) -> dict[str, Any]:
        ...
        cb_handler = CustomHtmlCallbackHandler()
        cb_manager = CallbackManager(handlers=[cb_handler])
        acs_tool = Tool(
